...
class ModificationApi {
  ...
  modifyReserveInfo() {
    ...
    $.ajax({
      async: false,
      type: "PUT",
      url: `http://127.0.0.1:8000/api/user/${checkObj.reserveId}`,
      contentType: "application/json",
      data: JSON.stringify(Modifyvalue),
      dataType: "json",
      success: (response) => {
        console.log("put 성공");
        successFlag = true;
...
class ReservationModificationService {
  ...
  setModifyValue() {
    ...
    Modifyvalue.reserveDate =
      document.getElementById("dateSelect").value;
    Modifyvalue.reserveTime =
      modifytime.options[modifytime.selectedIndex].text;
    Modifyvalue.adult =
      modifyadult.options[modifyadult.selectedIndex].text;
    Modifyvalue.child =
      modifychild.options[modifychild.selectedIndex].text;
  }
...
class checkReserveEvent {
  ...
  loadCheckData() {
    const checkData =
      ModificationApi.getInstance().Modify_getReserveData();
    const checkContents1 = document.querySelector(
      ".reserve-contents1 tbody"
    );
    const checkContents2 = document.querySelector(
      ".reserve-contents2 tbody"
    );
    const checkContents3 = document.querySelector(
      ".reserve-contents3 tbody"
    );
    const checkContents4 = document.querySelector(
      ".reserve-contents4 tbody"
    );
    ...
    checkData.forEach((data) => {
      document.querySelector(".modification-input-request").value =
        data.request;
    });
  }
}

class updateReserveEvent {
  static #instance = null;
  static getInstance() {
    if (this.#instance == null) {
      this.#instance = new updateReserveEvent();
    }
    return this.#instance;
  }
}
